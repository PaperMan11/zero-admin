...
func NewAddLogMiddleware(Sys operatelogservice.OperateLogService) *AddLogMiddleware {
	return &AddLogMiddleware{OperateLogService: Sys}
}
...
func (m *AddLogMiddleware) Handle(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = m.OperateLogService.AddOperateLog(r.Context(), &sysclient.AddOperateLogReq{
			Title:             "",
			OperationType:     "",
			OperationName:     convert.ToString(uid),
			RequestMethod:     r.Method,
			OperationUrl:      uri,
			OperationParams:   string(body),
			OperationResponse: responseBoy,
			OperationStatus:   int32(recorder.statusCode),
