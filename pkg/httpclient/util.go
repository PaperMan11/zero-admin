...
const (
	// read resp body err, should re-call doHTTP again
	_StatusReadRespErr = -204
	// do req err, should re-call doHTTP again
	_StatusDoReqErr = -500
)
...
func doHTTP(ctx context.Context, method, url string, payload []byte, opt *option) ([]byte, int, error) {
	...
	if err != nil {
		...
		if opt.logger != nil {
			opt.logger.Errorf("doHTTP got err: %s", err)
		}
...
// append url.Values into url string
